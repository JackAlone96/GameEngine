...
int main(int argc, char* argv[])
{
	...
	// the imgui color {r,g,b} wheel requires float from 0-1 instead of ints from 0-255
	...
	//if (!myFont.loadFromFile((std::filesystem::current_path() / "fonts\\TECHFONT.TTF").string()))
	//{
	...
	// set up a char array to set the text
	char displayString[255] = "Sample Text";

	// main loop - continues for each frame while windows is open
	while (window.isOpen())
	{
		// event handling
		sf::Event event;
		while (window.pollEvent(event))
		{
			...
			if (event.type == sf::Event::KeyPressed)
			{
				...
				std::cout << "KeyPressed with code = " << event.key.code << std::endl;
		...
		window.display(); //call the window display
	}
